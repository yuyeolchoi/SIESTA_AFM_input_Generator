...
`--filter-elements Ni Co`를 사용하면 선택한 원소의 스핀 색과 화살표만 표시됩니다.
선택하지 않은 원소도 원자 자체는 사라지지 않고 회색 비자성 마커로 남습니다.
`--show-bonds`는 ASE 공유결합 반지름 기반 결합선을 표시하며,
`--bond-radius-scale`(기본값 `1.0`)로 판정 범위를 조절할 수 있습니다. 주기 셀 경계를
넘는 결합은 표시되지 않습니다. non-collinear 화살표는 `DM.InitSpin`의 theta/phi
방향을 따르지만, up/down 색상은 의도적으로 기존 스칼라 moment의 부호만 사용하므로
순수 면내 moment도 spin-up 색으로 보일 수 있습니다.
...
- `manual-spins`: `--spin-values` 또는 `--spin-values-file`의 원자별 부호 있는 값을 그대로 보존합니다. 사용자 명시 배정이므로 후보 enumeration에서는 제외됩니다.
...
`--layer-per-species`를 주면 각 원소마다 독립 좌표 스택을 만들고 그 안에서 층 부호를
교대합니다. 이 옵션은 `--method layer`에서만 쓸 수 있고 `--layer-direction`에도
적용됩니다. 기본값은 기존의 결합 스택입니다. 다원소 스피넬 ferrimagnet에는
`by-coordination`을 우선 사용하고, 원소별 독립 layer 패턴이 물리적으로 의도된
경우에만 이 옵션을 사용하십시오.
...
`examples/Fe_CO5_homogeneous_catalyst.xyz` 같은 분자 XYZ 입력은 셀이 없는
비주기 구조로 읽습니다. 입력에 실제로 의미 있는 nonsingular cell이 있지 않다면
`--slab`이나 `--periodic-axes`는 필요하지 않습니다. `analyze`, `validate`, `plot`,
GUI와 Cartesian 좌표를 쓰는 생성 방법은 XYZ의 원자 순서를 그대로 사용합니다.
각 방법에 필요한 일반 옵션이 올바르다면 `alternating-index`, `random`, Cartesian
`layer`, `checkerboard`, neighbor graph 방법, `graph-coloring`, `manual-groups`,
`by-species`, `by-coordination`, `manual-spins`가 동작합니다.

반면 fractional 좌표를 명시적으로 요구하는 방법은 셀이 없는 XYZ에 쓸 수 없습니다.
fractional `propagation-vector`(A/C/G preset 포함)와
`layer --fractional-layers`는 nonsingular-cell 오류를 명확히 냅니다.
`--cartesian-coordinates`를 주면 propagation vector를 Cartesian 좌표로 해석할 수
있지만, 의도한 분자 모델에 맞는 경우에만 사용하십시오. 결정학적 symmetry dedup도
셀이 있는 구조용입니다.

작은 착물에서는 `manual-spins`로 1-based 원자 인덱스에 부호 있는 moment를 직접
지정할 수 있습니다.
...
위 값은 입력 형식 예시이며 Fe(CO)5의 바닥상태 스핀을 주장하는 값이 아닙니다.
자기 중심이 여러 개면 `--spin-values 1=+4.0 7=-2.0 9=+1.0`처럼 지정합니다.
선택된 모든 자기 원자를 써야 하며, 생략한 원자를 0으로 둘 때만
`--fill-unspecified-zero`를 사용합니다. CSV는 별도의 signed schema를 사용하며
`--spin-values-file`로 읽습니다.
...
`--spin-values`와 `--spin-values-file`은 동시에 쓸 수 없고,
`--magnetic-species`로 선택되지 않은 실제 원소의 인덱스를 지정하면 오류입니다.
이 signed 경로는 기존 `--site-moment-file`과 의도적으로 분리되어 있습니다. 기존
site moment CSV는 크기를 지정하므로 음수 `moment`의 부호는 계속 무시되고 method가
부호를 결정합니다.
...
`generate`와 `make-input`에서 `--spin-mode non-collinear`를 사용하면 각 graph color를
`theta=90°`, `phi=360° * color / n_colors`인 면내 방향으로 매핑합니다. 따라서 3색
그래프는 0°/120°/240° 질서를 만듭니다. 이 모드는 `--method graph-coloring`에서만
사용할 수 있고 `--color-spins`와 함께 지정할 수 없습니다. 기본값은 기존과 바이트 단위로
동일한 collinear 출력입니다.
...
`Batch workflow` 탭은 여러 초기 자기 상태의 total energy를 비교하는 선택 기능이며,
...
3D 미리보기와 결과 노트북 사이의 가로 구분선을 드래그하면 `Batch workflow`에 더 많은
세로 공간을 줄 수 있습니다. 이 워크플로의 세 서브탭에는 각각 세로 스크롤바와 마우스 휠
스크롤이 제공됩니다. 포인터가 후보 또는 결과 표 위에 있을 때는 바깥 탭 대신 해당 표가
스크롤됩니다.
...
`manual-spins`를 선택하면 원자 수 60개 이하 구조에는 원자 순서의
`atom index | element | spin` 표가 추가되고, spin 셀을 더블클릭해 부호 있는 값을
편집합니다. 편집은 기존 디바운스된 실시간 미리보기 경로를 사용하므로 현재 3D 카메라도
유지됩니다. 60개를 넘는 구조에서는 수백 행 표 대신 `atom_index,spin` CSV 선택 UI가
나타납니다.
...
현재 구조 위에서 열 수 있습니다. 3D 미리보기의 원자 인덱스는 60개 이하 구조에서만
기본 표시되고 더 큰 구조에서는 숨겨집니다. `Show atom indices`로 현재 구조의 표시 여부를
언제든 바꿀 수 있습니다. 미리보기에는 구조에 실제로 존재하는 원소별 스핀 표시
체크박스와 `Show bonds`, 결합 반지름 스케일도 제공됩니다. 생성하거나 스핀 파일을 연 뒤
각 원소 체크박스 옆에는 배정된 up/down/zero(미지정 포함) 자리 수가 표시됩니다. 이 개수는
체크 상태와 무관하게 유지되며, 원소 체크를 해제해도 원자 마커는 남고 스핀 색과 화살표만
숨겨집니다. `by-coordination` 결과에는 검출된 CN별 체크박스 행도 추가됩니다. 원소와 CN
필터는 AND로 결합되므로 두 체크박스가 모두 선택된 자리의 스핀만 표시됩니다. 다른
method이거나 저장된 CN 정보가 없는 결과에는 CN 컨트롤이 나타나지 않습니다. 결합선은
원자 수 60개 이하 구조에서만 기본으로 켜집니다.
...
CN과 sublattice는 `-`로 표시합니다. 기본 행 순서는 입력 원자 인덱스 순서이며, `CN` 열
헤더를 클릭하면 CN 오름차순으로 묶이고 다시 클릭하면 원자 인덱스 순서로 돌아옵니다.
...
완전한 실행 시작 FDF를 만들려면 `Primary actions` 고정 바에 있는 눈에 띄는
...
spinel 구조가 아니므로 `by-coordination` 검증 모델로 사용하면 안 됩니다. 이 파일의
Ni/Co-O 거리는 의도적으로 일반적인 공유결합 판정 거리보다 길게 되어 있어
`--show-bonds`/`Show bonds`를 적용해도 결합이 거의 표시되지 않습니다. 원소 필터와
결합선 기능을 물리적으로 타당한 다원소 구조에서 테스트하려면 대신
`examples/NiCo2O4_spinel_demo.cif`를 사용하세요 (COD 1538531 실제 Co3O4 spinel
구조에서 8개의 사면체 Co 자리를 Ni로 치환한 구조입니다).
...
좌표 행과 `DM.InitSpin` 인덱스는 입력 원자 순서와 정확히 일치합니다. 배위별 분리를
사용하지 않으면 FDF/XV의 species ID는 보존하고, 다른 형식은 원소가 처음 등장한
순서대로 ID를 부여합니다.
...
`--method by-coordination`에서는 opt-in 옵션
`--split-species-by-coordination`으로 같은 자기 원소의 서로 다른 CN 자리를 별도의
연속 species ID로 나눌 수 있습니다. 좌표 행 순서는 바뀌지 않고 4번째 species ID만
달라지며, 기존 `Co_2`, `Co_3` 라벨 규칙과 `ChemicalSpeciesLabel`의 CN/배위기하
주석을 사용합니다. 이때 `--hubbard-u Co@4=3.0 Co@6=5.0`처럼 CN별 U를 지정할 수
있습니다. plain `Co=3.0`은 분리된 모든 Co species에 같은 U를 적용합니다. 각 라벨에
대응하는 pseudopotential 파일이 필요하므로 실행 디렉터리에 `Co_2.psf`와
`Co_3.psf` 사본(또는 같은 이름의 PSML 파일)을 함께 두어야 합니다. 플래그가 없으면
기존 출력이 유지되며 `@CN` Hubbard U는 오류가 됩니다. GUI에서는
`by-coordination` 방법을 선택했을 때만 같은 opt-in 체크박스가 활성화됩니다.
